...
class Post(object):
    ...
    def __init__(self, thread, data):
        self._thread = thread
        self._data = data
        self._url = Url(board_name=self._thread._board.name, domain_name=thread._domain, https=thread.https)		# 4chan URL generator
    ...
    @property
    def subject(self):
        return self._data.get('subnum')

    @property
    def html_comment(self):
        return self._data.get('comment_processed', '')

    @property
    def text_comment(self):
        return self._data.get('comment_sanitized', '')
    ...
    @property
    def spoiler(self):
        return self._data.get('media').get('spoiler') == 1
    ...
    @property
    def has_file(self):
        return 'filename' in str(self._data)
