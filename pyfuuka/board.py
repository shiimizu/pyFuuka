...
def get_boards(board_name_list, *args, **kwargs):
    """Given a list of boards, return :class:`pyfuuka.Board` objects.

    Args:
        board_name_list (list): List of board names to get, eg: ['b', 'tg']

    Returns:
        dict of :class:`pyfuuka.Board`: Requested boards.
    """
...
def get_all_boards(*args, **kwargs):
    """Returns every board on FFuuka.

    Returns:
        dict of :class:`pyfuuka.Board`: All boards.
    """
...
class Board(object):
    ...
    """
    def __init__(self, board_name, domain_name, https=False, session=None):
        """Creates a :mod:`pyfuuka.Board` object.

        Args:
            board_name (string): Name of the board, such as "tg" or "etc".
            https (bool): Whether to use a secure connection to pyfuuka.
        ...
        self._url = Url(board_name=board_name, domain_name=self._domain_name, https=self._https)

        self._requests_session = session or requests.session()
        self._requests_session.headers['User-Agent'] = 'py-pyfuuka/%s' % __version__
    ...
    def get_thread(self, thread_id, update_if_cached=True, raise_404=False):
        """Get a thread from pyfuuka via pyfuuka API.
        ...
        Returns:
            :class:`pyfuuka.Thread`: Thread object
        """
        ...
        # To get full complete JSON
        url = self._url.thread_api_url(
                thread_id = thread_id
                )
        self._requests_session.headers.update({'User-Agent': self.user_agent})
        ...
        # Print API call
        #print(self._url.thread_api_url( thread_id = thread_id ))
        #print(res.content)
        #print()
    ...
    def get_threads(self, page=1):
        ...
        Pages on pyfuuka are indexed from 1 onwards.

        Args:
            page (int): Page to request threads for. Defaults to the first page.

        Returns:
            list of :mod:`pyfuuka.Thread`: List of Thread objects representing the threads on the given page.
        """
    ...
    def get_all_threads(self, expand=False):
        ...
        Returns:
            list of :mod:`pyfuuka.Thread`: List of Thread objects representing every thread on this board.
        """
        if not expand:
    ...
    @property
    def user_agent(self):
        return "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.87 Safari/537.36"
